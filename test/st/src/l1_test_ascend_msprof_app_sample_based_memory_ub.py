...
class TestAscendMsprofAppSampleBasedMemoryUB(BaseAscendMsprofChecker):
    def check_ai_vector_core_utilization_data(self):
        ...
        FileChecker.check_csv_data_non_negative(ai_vector_core_utilization_path[0],
                                                ...
                                                columns=["ub_read_bw_vector(GB/s)", "ub_write_bw_vector(GB/s)",
...
def test_ascend_msprof_app_sample_based_memory_ub(prof_path):
    expect_db_tables = ["ACC_PMU", "AICORE_FREQ", "CANN_API", "COMPUTE_TASK_INFO", "ENUM_API_TYPE",
                        ...
                        "STRING_IDS", "TASK"]
    test_ascend_msprof = TestAscendMsprofAppSampleBasedMemoryUB(prof_path, expect_db_tables)
    test_ascend_msprof.check_msprof_file()


if __name__ == '__main__':
    path = sys.argv[1]
    test_ascend_msprof_app_sample_based_memory_ub(path)
