...
class TestAscendMsprofAppTaskBasedPipeUtilization(BaseAscendMsprofChecker):
...
def test_ascend_msprof_app_task_based_pipe_utilization(prof_path):
    ...
    test_ascend_msprof = TestAscendMsprofAppTaskBasedPipeUtilization(prof_path, expect_db_tables)
    test_ascend_msprof.check_msprof_file()


if __name__ == '__main__':
    path = sys.argv[1]
    test_ascend_msprof_app_task_based_pipe_utilization(path)
